...
void initCurrentSense(void)
{
	...
	debugWriteString("entering currentSenseConfig\n");
	currentSenseConfig();
...
void currentSenseConfig(void)
{
	...
	currentSenseConfigDevice(CURRENT_DAUGHTER);
	currentSenseConfigDevice(CURRENT_FPGA_VCCIO);
	currentSenseConfigDevice(CURRENT_SRAM);
...
void currentSenseConfigDevice(uint8_t device)
{
	debugWriteString("current sense configuring: ");
	debugWriteHex8(device);
	debugWriteString("\n");
	cli();
				...
				case CURRENT_WIRELESS:
					switch(currentByte)
					{
						...
						case 3:
							...
							currentDevice = CURRENT_DAUGHTER;
...
//                            currentState = I2C_IDLE;
                ...
                case CURRENT_SRAM:
                case CURRENT_FPGA_VCCIO:
                    switch(currentByte)
                    {
                        // VCCAUX H
                        case 0:
                            measurement.breakdown.sram = data << SENSE_SHIFT_UP;
                            i2c_readAck_nonblocking();
                            break;
                            // VCCAUX L
                        case 1:
                            measurement.breakdown.sram |= data >> SENSE_SHIFT_DOWN;
                            i2c_readAck_nonblocking();
                            currentDevice = CURRENT_FPGA_VCCIO;
                            break;
                            // VCCINT H
                        case 2:
                            measurement.breakdown.vccio = data << SENSE_SHIFT_UP;
                            i2c_readNak_nonblocking();
                            break;
                            // VCCINT L
                        case 3:
                            measurement.breakdown.vccio |= data >> SENSE_SHIFT_DOWN;
                            i2c_stop();
...
currentMeasurementFloat fetchCurrentMeasurementFloat(void)
{
	...
	output.breakdown.sram = fetchCurrent(CURRENT_SRAM);
	output.breakdown.vccio = fetchCurrent(CURRENT_FPGA_VCCIO);
	output.breakdown.daughter = fetchCurrent(CURRENT_DAUGHTER);
...
void printCurrentMeasurement(currentMeasurement c)
{
	...
	for(int i = CURRENT_USB; i <= NUM_MEASUREMENTS; i++)
	{
...
uint8_t getAddress(uint8_t device)
{
	switch(device)
	{
	    ...
	    case CURRENT_SRAM:
	        return CURRENT_FPGA_SRAM_ADDRESS;
	    case CURRENT_FPGA_VCCIO:
	        return CURRENT_FPGA_VCCIO_ADDRESS;
	    case CURRENT_DAUGHTER:
	        return CURRENT_DAUGHTER_ADDRESS;
		case CURRENT_WIRELESS:
...
uint8_t getChannel(uint8_t device)
{
	switch(device)
	{
		...
		case CURRENT_DAUGHTER:
			return CURRENT_DAUGHTER_CHANNEL;
		case CURRENT_WIRELESS:
        ...
        case CURRENT_FPGA_VCCIO:
            return CURRENT_FPGA_VCCIO_CHANNEL;
	    case CURRENT_SRAM:
	        return CURRENT_FPGA_SRAM_CHANNEL;
		case CURRENT_UNKNOWN:
		default:
			return 0;
	}
...
float getR(uint8_t device)
{
	switch(device)
	{
	    ...
	    case CURRENT_DAUGHTER:
	        return CURRENT_DAUGHTER_R;
//		case CURRENT_MONITOR:
//			return CURRENT_MONITOR_R;
	    case CURRENT_FPGA_VCCIO:
            return CURRENT_FPGA_VCCIO_R;
	    case CURRENT_SRAM:
	        return CURRENT_FPGA_SRAM_R;
		case CURRENT_WIRELESS:
